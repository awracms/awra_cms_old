...
class SpatialFileNode(InputNode):
    def __init__(self,filename,variable=None,preload=False):
        ...
        self.fh = _spatial_file_loader(filename,variable)
...
def _spatial_file_loader(fn,variable):
    from os.path import splitext

    ext = splitext(fn)[1]

    if ext == '.flt':
        return GDALSpatialFile(fn)
    elif ext in ['.nc','.h5']:
...
class HDF5SpatialFile:
    def __init__(self,fn,variable):
        ...
        self.dims = [k.label for k in self.fh[variable].dims]
        self.data = self.fh[variable]
        #self.data = NanFiller(self.fh[variable],self.fh[variable].fillvalue)

    def get_extent(self):
        # +++ Currently assumes no mask
        return extents.from_latlons(self.fh['dimensions']['latitude'][...],self.fh['dimensions']['longitude'][...])
...
class GDALSpatialFile:
    ...
    def get_extent(self):
        georef,mask = geo.get_georef_gridobj(self.ds)
        return extents.Extent(georef,mask)
        
    def get_coords(self):
        georef,_ = geo.get_georef_gridobj(self.ds,False)

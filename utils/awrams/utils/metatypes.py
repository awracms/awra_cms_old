...
from .general import pretty_format_dict
...
def iter_build_objectdict(in_dict):
    '''
    Build ObjectDict iteratively (ie child dictionaries are 
        also converted to objectdicts)
    '''
    obj_type = objectify_dict_type(type(in_dict))
    od = obj_type()
    for k,v in list(in_dict.items()):
        if isinstance(v,dict):
            od[k] = iter_build_objectdict(v)
        else:
            od[k] = v
    return od

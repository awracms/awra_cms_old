...
_MASK_FILE = os.path.join(os.path.dirname(__file__),'../awrams/utils/data/mask.h5')
...
# @nottest
def test_extent_all():
    e = extents.Extent.from_file(_MASK_FILE)
...
#    gsd = extents.Extent.from_file(_MASK_FILE)
    full = extents.ExtentFactory(extents.Extent.from_file(_MASK_FILE))
    ...
    #    c.compute_areas()
        assert_almost_equal(c.area,27956330.541286,places=6)
        print("300,200",c.area)

        #c = gsd.extent_from_cell_offset(300,400)

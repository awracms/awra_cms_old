...
def resample_to_months_df(in_dct, how, rescale=True):
    ...
    for col_name in list(in_dct.keys()):
        ...
        try:
            valid_idx = filter_months(valid_only(cur_data),min_month_len=cfg.MONTHLY_REJECTION_THRESHOLD)
    ...
    if len(out_dct.keys()) == 0:
        logger.warning("No valid data for monthly freq using MONTHLY_REJECTION_THRESHOLD=%d",cfg.MONTHLY_REJECTION_THRESHOLD)
    return out_dct

def resample_to_years_df(in_dct, how, min_months=cfg.ANNUAL_REJECTION_THRESHOLD, annual_rule='A'):
    """

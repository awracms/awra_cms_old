...
class InputDef:
    ...
    agg_method can (currently) be 'mean' or 'volume'
    '''
...
class CalibrationResults:
    ...
    def get_best_paramset(self,global_key=None,reverse=False):
        '''
        Get the parameter set with the best (minimum) value in <global_key>
        '''
        bpi = self.best_param_index(global_key,reverse)
        return pd.Series(self.ds['parameter_values'][bpi],index=self.parameter_names)